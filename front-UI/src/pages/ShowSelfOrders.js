...
export default function ShowSelfOrders() {
  ...
  const acceptApproveOrder = () => {
    approveOrderById(approveOrderId).then(() => {
      const updatedOrders = selfOrders.filter(
        (order) => order.id !== approveOrderId
      );
  ...
  const acceptDeleteOrder = () => {
    withdrawOrderById(deleteOrderId).then(() => {
      const updatedOrders = selfOrders.filter(
        ...
        <Table>
          <TableHead sx={{ backgroundColor: theme.palette.primary.main }}>
            <TableRow>
              ...
              <TableCell sx={{ color: "white" }}>
                {t("medication_quantity")}
              </TableCell>
              <TableCell sx={{ color: "white" }}>
                {t("order_total_price")}
              </TableCell>
              <TableCell sx={{ color: "white" }}>
                {t("prescription_number")}
              </TableCell>
              <TableCell sx={{ color: "white" }}>
                {t("approve_order")}
              </TableCell>
        ...
        <Table>
          ...
          <TableBody>
            {selfOrders.map((item, index) => {
              const totalPrice = item.orderMedication.reduce(
                (sum, medication) => {
                  return (
                    sum +
                    medication.medication.currentPrice * medication.quantity
                  );
                },
                0
              );

              const isOrderToBeApproved =
                ...
                <React.Fragment key={`${index}-${medicationIndex}`}>
                  {medicationIndex === 0 && (
                    <TableRow>
                      ...
                      <TableCell>
                        {medication.medication.currentPrice}
                      </TableCell>
                      ...
                      {item.prescription && medicationIndex === 0 ? (
                        <TableCell rowSpan={item.orderMedication.length}>
                          {item.prescription.prescriptionNumber}
                        </TableCell>
                      ) : (
                        <TableCell rowSpan={item.orderMedication.length}>
                          -
